    ...
    internal class ProfileService : IProfileService
    {
        ...
        */
        ...
        private async Task<IEnumerable<Claim>> GetClaimsFromAccountAsync(User user)
        {
            ...
            {
                claims.AddRange((await userManager.GetRolesAsync(user)).Select(x => new Claim(JwtClaimTypes.Role, x)));
            }
        ...
        //{
        //    var claims = await GetClaimsFromAccountAsync(user);
        //    return claims.Where(x => x.Type == JwtClaimTypes.Name).Select(x => x.Value).FirstOrDefault()
        ...
        {
            return principal.Claims.Where(x => x.Type == JwtClaimTypes.Subject).Select(x => x.Value).FirstOrDefault()
